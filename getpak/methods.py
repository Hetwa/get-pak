...
class Methods:
    ...
    def classify_owt_spm_weights(self, class_px, angles, n=1, remove_classes=None):
        """
        Attribute weights to the n-th most important OWTs, based on the spectral angle mapper
        The weights are used for calculating weighted means of the water quality parameters, in order to smooth the
        spatial differences between the pixels, and also to remove possible outliers generated by some models
        ...
        This function uses the results of classify_owt_spm_px as input data
        ...
        # be used
        owt_weights = np.zeros((n, *class_px.shape), dtype='float32')
        owt_classes = np.zeros((n, *class_px.shape), dtype='uint8')
        ...
        # Calculating the weights based on normalisation of the n+1 lowest Euclidean distance (parallel of convertion of
        # units) and assigning values to the matrices
        for i in range(n):
            owt_classes[i, nzero[0], nzero[1]] = indices[:, i] + 1  # summing one due to positioning starting in 1
            for j in range(nzero[0].shape[0]):
    ...
    def blended_turb(self, rrs_dict, owt_classes, owt_weights, limits=True):
        """
        Function to calculate a blended turbidity (turb) product based on turb matrices calculated with function turb
        in mode 'owt'

        Parameters
        ----------
        @rrs_dict: rrs_dict: a xarray Dataset containing the Rrs bands
        @owt_classes: an array with the different OWT classes calculated with function classify_owt_turb_weights
        @owt_classes: an array with the OWT weights calculated with function classify_owt_turb_weights
        @limits: boolean to choose whether to apply the limits for the different layers of turb products
        ...
        if isinstance(owt_classes, np.ndarray) and len(owt_classes.shape) == 3:
            # calculating turb for each
            aux_turb = np.zeros((owt_classes.shape[0], *rrs_dict['Red'].shape), dtype='float32')
            for i in range(0, owt_classes.shape[0]):
                aux_turb[i, :, :] = Methods.turb(self, rrs_dict, owt_classes[i, :, :], limits=True, alg='owt')

            # checking the limits
            if limits:
                for i in range(1, owt_classes.shape[0]):
                    over = np.where((np.absolute(aux_turb[i, :, :] - aux_turb[0, :, :]) > (4 * aux_turb[0, :, :])))
                    aux_turb[i, over[0], over[1]] = 0

            # inserting weights = 0
            for i in range(0, owt_classes.shape[0]):
                owt_weights[i, np.where(np.isnan(aux_turb[i, :, :]))[0], np.where(np.isnan(aux_turb[i, :, :]))[1]] = 0
                aux_turb[i, np.where(np.isnan(aux_turb[i, :, :]))[0], np.where(np.isnan(aux_turb[i, :, :]))[1]] = 0

            # calculating the blended product
            num = np.zeros(rrs_dict['Red'].shape, dtype='float32')
            den = np.zeros(rrs_dict['Red'].shape, dtype='float32')
            for i in range(0, owt_classes.shape[0]):
                num = num + (owt_weights[i, :, :] * aux_turb[i, :, :])
                den = den + owt_weights[i, :, :]
            turb = num / den

            # changing from np.nan to 0:
            turb[np.where(np.isnan(turb))] = 0

        elif isinstance(owt_classes, np.ndarray) and len(owt_classes.shape) == 3:
            print('There is only one class of OWT, use function Methods.turb instead.')
            sys.exit(1)
        else:
            print('Error: The vector of OWT classes has the wrong number of dimensions.')
            sys.exit(1)
